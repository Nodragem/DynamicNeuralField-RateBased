...
## --------------------
start_bar = 450; end_bar = 550 # one bump
#start_bar = 350; end_bar = 650 # one bump
#start_bar = 325; end_bar = 675 # one bump 
#start_bar = 315; end_bar = 685 # one bump ## size 370
#start_bar = 310; end_bar = 690 # two bumps ## size 380
#start_bar = 300; end_bar = 700 # two bumps
#start_bar = 250; end_bar = 750 # two bumps

# here we modified the Trappenberg set up of parameters:
GC1 = GeneratorConstante(250.0*1.0) 
GC1.color = [1,0,0]
GC2 = GeneratorConstante(250.0*0)  
width = 1000
NF = NeuralField1D(width, 100) ## time constant of 100 just to slow the process down and see it on the animation
NF.u0 = 0 ##Trappenberg biases the equilibirum state of build-up neurons
NF.getProbe(0).position = start_bar + 50
NF.getProbe(0).color = [1.0, 0.7, 0.2]
NF.getProbe(0).name = "P1"
NF.addProbe("P2", end_bar-50).color = [0.2, 0.7, 1.0]
NF.addProbe("P3", 500).color = [0.7, 0.2, 1.0]
...
NF.beta = 0.02 # 0.07 in Trappenberg
NF.firing_offset = 350 ## this allows the model to be stable when no input (quite important, isn't it?'), it can't fire for negative value of u(t), which seems quite reasonnable. However, this parameters was set to 0 in Trappenberg 2001

# But we can also change the gain function and use an inverse exponential:
def exponentialRise(u):
    tau_fr = 200.0
    u_offset = 250 # default = 150 
    #### with bar size = 380; 
    ## u_offset 250: total suppression 
    ## u_offset 243.525 some very very weak
    ## u_offset 237.5: some very weak
    ## u_offset 225: some
    ## u_offset 200: some
    ## u_offset 150 : some suppression
...
Sa = 60.0; A = 144.0/200/5;  ## Sa = 0.6mm while 1 neurons is 0.01
Sb = 180.0; B = 44.0/100/5;
c = 16.0/100/5;
...
#w1 = np.zeros(width); w1[390:410] = 1.0
w1 = np.zeros(width); w1[start_bar:end_bar] = 1.0
W1 = NF.set_external(w1, GC1)  
w2 = np.zeros(width); w2[590:610] = 1.0
...
displayed_probes = ["Default", "P1", "P2", "P3"]
...
ax2.hlines(0, start_bar, end_bar, color= "black", linewidth = 2.5)
